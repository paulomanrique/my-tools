...
export function ToolShell({ locale, toolId, title, description, children }: ToolShellProps) {
  ...
  const repositoryUrl = 'https://github.com/paulomanrique/my-tools'
    ...
    <div className="min-h-screen p-4 text-ink-50 md:p-6">
      <div className="mx-auto grid max-w-7xl gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
        <div className="space-y-4 lg:sticky lg:top-6 lg:h-fit">
          <div className="px-1">
            <LanguageSwitcher locale={locale} toolId={toolId} showLabel={false} />
          </div>

          <aside className="panel glass-cut p-4 lg:p-6">
                ...
                {TOOLS.map((tool) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-sm leading-6 text-ink-200/70">{t(`${section}.short`)}</p>
                    </Link>
                  )
                })}
              </nav>
